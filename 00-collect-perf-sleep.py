...
def get_perf_idle_samples(gov):
    ...
    cmd = f"sudo perf record -a -e sched:sched_switch,power:cpu_frequency,power:cpu_idle,irq:irq_handler_entry,timer:hrtimer_cancel,power:cpu_idle_miss -o {gov}/perf.data -- sleep 120"
    exec_cmd(cmd)
    cmd = f"sudo chown {os.getuid()}:{os.getgid()} {gov}/perf.data"
    exec_cmd(cmd)
    cmd = f"perf script -i {gov}/perf.data -s ~/Worky/PM/Scripts/linux-kernel-perf/tools/perf/scripts/python/power-statistics.py -- --mode idle-governor --csv --file-out"
    exec_cmd(cmd)
